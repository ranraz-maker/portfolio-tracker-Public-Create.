...
def yahoo_quotes(symbols: tuple[str, ...]) -> dict[str, tuple[float,float]]:
    ...
    for d in r.json().get("quoteResponse", {}).get("result", []):
        sym = d.get("symbol"); price = d.get("regularMarketPrice"); prev = d.get("regularMarketPreviousClose", price)
        if sym is not None:
            out[sym] = (float(price) if price is not None else float("nan"),
...
def stooq_quotes(symbols: tuple[str, ...]) -> dict[str, tuple[float,float]]:
    ...
    rev = {v: k for k, v in mapped.items()}; out = {}
    for row in reader:
        sym = row.get("Symbol"); orig = rev.get(sym, sym)
        try:
            close = float(row.get("Close") or "nan"); prev = float(row.get("Open") or "nan")
        except ValueError:
...
def fetch_quotes(symbols: list[str]) -> dict[str, tuple[float,float]]:
    ...
    try:
        for i in range(2):
            try:
                yahoo_out = yahoo_quotes(syms_tuple); break
...
def yahoo_history(symbol: str, start_dt: datetime, end_dt: datetime, interval: str = "1d") -> pd.Series:
    ...
    res = res[0]; closes = res.get("indicators", {}).get("quote", [{}])[0].get("close", [])
    ts = res.get("timestamp", []); if not closes or not ts: return pd.Series(dtype="float64")
    idx = pd.to_datetime(ts, unit="s", utc=True).date
    return pd.Series(closes, index=pd.Index(idx, name="date"), dtype="float64").dropna()

# ---------- Fetch history for window (FIRST) ----------
hist_map = {}
for t in TICKERS:
    try:
        s = yahoo_history(t,
...
prices = pd.DataFrame(hist_map).dropna(how="any")  # aligned by common dates

# ---------- Fetch live quotes (for optional live table) ----------
try:
...
# ---------- HOLDINGS TABLE (now follows timeframe by default) ----------
table_mode = st.radio("Holdings table mode", ["Timeframe (start→end)", "Live snapshot (now)"], horizontal=True, index=0)

def render_table_timeframe():
    if prices.empty:
    ...
    tot_start = float(df["Start Value $"].sum())
    tot_end   = float(df["End Value $"].sum())
...
tbl_shown = render_table_timeframe() if table_mode.startswith("Timeframe") else render_table_live()
...
if prices.empty:
    ...
    units = {t: ((total_usd * weights[t]) / first_row[t]) if (t in prices.columns and first_row[t] and not math.isnan(first_row[t]) and first_row[t] > 0) else 0.0
             for t in TICKERS}
    ...
    if ret_series.empty:
        ...
        last_val = float(port_val.iloc[-1]); rets = ret_series.values
        sims_end, sims_ret = [], []
        ...
        sims_end = np.array(sims_end); sims_ret = np.array(sims_ret)
        ...
        st.markdown("**Risk / Return summary**"); st.dataframe(summary, use_container_width=True)
        st.markdown("**Threshold probabilities**"); st.dataframe(thresh, use_container_width=True)
